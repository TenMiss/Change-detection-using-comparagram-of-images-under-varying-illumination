...
figure,imshow(im2)
% both images are assumed to be of the same size
im1 = rgb2gray(im1);
im2 = rgb2gray(im2);
[m,n] = size(im1); 
...
for ii = 0:(mblocks - 1)    % indices in both i- and j-directions 
  for jj = 0:(nblocks - 1)  
    ...
    chmask(ii * mb + rows, jj * mb + cols) = imabsdiff(xx, yy); % simple differencing is performed between the
                                                                % first image and the normalized second image
   end;  
end;

chmask = mat2gray(chmask);  % converting the matrix into a gray-scale image
chmask = im2bw(chmask, th); % thresholding the gray-scale image to obtain the 
