...
% Description:  implements Hsu's polynomial fitting, quadratic model.
...
% Function value matrix A
    ...
    paramxx = C\A; %least-squares solution
                   % paramxx is a vector containing the quadratic model parameters for block xx
    ...
    % finding the error variances with quadratic model fitting
    ...
    % fitting the same quadratic model for both the blocks        
    C2 = [C;C];
    E = [A;B]; 
    paramxy = C2\E; %least-squares solution
                    % paramxy is a vector containing the quadratic model parameters when the same quadratic model 
                    % is fit to both the blocks

    % error variance from both the blocks when same quadratic model is fit to both the blocks
